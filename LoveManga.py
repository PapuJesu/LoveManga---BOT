...
@tasks.loop(time=time(hour=11, minute=7))
# ─── Para pruebas  ─── 
...
async def mensaje_programado():
    ...
    if canal:
        await canal.send(f"📅 Día **{dia_actual + 1}** diciendo pene por cada día que pase hasta que <@{USER_KENE}> prenda stream.")
        for _ in range(dia_actual + 1):
